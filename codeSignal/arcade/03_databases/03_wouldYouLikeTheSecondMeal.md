# Would you LIKE the Second Meal <br><br><br>
...
BEGIN
	SELECT id, name, surname FROM Suspect WHERE name LIKE 'b%' AND surname LIKE 'gre_n' AND height <= 170;
END
```
...
BEGIN
	SELECT id, name, surname FROM Suspect WHERE name NOT LIKE 'b%' OR surname NOT LIKE 'gre_n' OR height <= 170;
END
```
...
BEGIN
	SELECT * FROM users WHERE attribute LIKE BINARY CONCAT('%_\%%', first_name, '\_', second_name, '%\%%') ORDER BY attribute;
END
```
